...
# --- WIDGET AUXILIAR PARA O OLHINHO ---
...
class LoginScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # --- MODIFICAÇÃO AQUI ---
        # Transformado em Botão para ser clicável
        help_button = Button(
            ...
            size_hint_y=None,
            height=dp(30),
            background_normal='',       # Fundo transparente
            background_color=(0, 0, 0, 0),  # Fundo transparente
            on_press=self.go_to_help    # Ação de clique
        )
        layout_principal.add_widget(help_button)
        # --- FIM DA MODIFICAÇÃO ---
...
class MainScreen(Screen):
    ...
    def create_info_grid(self):
        ...
        grid.add_widget(self.InfoCard('Tem aproximadamente', '30', 'pessoas na sala'))
    ...
    def toggle_power_state(self, instance):
        ...
        if card.is_on:
            ...
            self.temperature_label.text = "25°"
    ...
    def __init__(self, **kwargs):
        ...
        with self.rounded_image_widget.canvas.before:
            ...
            self.rounded_image_rect = RoundedRectangle(
                ...
                radius=[dp(25)] # Raio das bordas (você tinha mudado para 15, voltei para 25)
            )
...
class HelpScreen(Screen):
    def __init__(self, **kwargs):
        ...
        btn_voltar.bind(on_press=self.go_back_to_login) # Vai para o login
        barra_superior.add_widget(btn_voltar)
        container.add_widget(barra_superior)
        ...
        container.add_widget(Label(
            text='Procedimento\nde Acesso',
            font_name='FuturaNegrito',
            font_size='45sp', # Ajustado para caber
            color=get_color_from_hex('#292929'),
            halign='left',
            size_hint_y=None,
            height=dp(120),
        ...
        with card.canvas.before:
            Color(rgb=get_color_from_hex('#d0d0d0')) # Cor do card
            card.rect = RoundedRectangle(pos=card.pos, size=card.size, radius=[dp(20)])
        card.bind(pos=self._update_rect, size=self._update_rect)
        
        # Texto formatado com [b] para negrito
        texto = (
            "[b]Prezado usuario,[/b]\n"
            "Para acessar o sistema vinculado ao seu dispositivo ESP, por favor, siga atentamente as "
            "etapas abaixo para garantir a conexao e a autenticacao bem-sucedidas.\n\n"
            "[b]Passos para o Login:[/b]\n"
            "[b]1. Identificacao do Dispositivo:[/b] Primeiramente, localize e anote o endereço IP do "
            "seu dispositivo ESP. Este endereco e essencial para a comunicacao na rede.\n\n"
            "[b]2. Verificacao da Conexao:[/b] Assegure-se de que o seu dispositivo (computador ou "
            "smartphone) está conectado à mesma rede de internet que foi configurada no ESP. "
            "Ambos os dispositivos precisam estar na mesma rede local para se comunicarem.\n\n"
            "[b]3. Autenticacao no Sistema:[/b] Na tela de login, preencha os campos de texto "
            "solicitados, inserindo o endereço IP do ESP (anotado no passo 1) e o nome do seu "
            "comodo conforme cadastrado.\n\n"
            "Ao seguir estas instrucoes, o acesso ao sistema devera ser concluido com sucesso.\n\n"
            "[b]Suporte[/b]\n"
            "[b]Caso haja problemas durante o acesso, verifique o manual do dispositivo ou entre "
            "em contato conosco através do e-mail: airflow.ifrn@gmail.com[/b]"
        )
        
        text_container = BoxLayout(orientation='vertical', padding=[dp(20), dp(20), dp(20), dp(20)], size_hint_y=None)
        lbl = Label(
            text=texto,
            markup=True, # Habilita o [b]
            font_name='Futura',
            font_size='15sp',
            color=get_color_from_hex('#555555'),
            halign='left',
            valign='top',
            size_hint_y=None,
            text_size=(dp(280), None) # Largura do texto
        )
        lbl.bind(texture_size=lambda inst, size: setattr(lbl, 'height', size[1]))
        text_container.add_widget(lbl)
        text_container.bind(minimum_height=text_container.setter('height'))
    ...
    def go_back_to_login(self, instance): # Função para voltar ao login
        if self.manager:
...
class ProjetoIntegradorApp(App):
    def build(self):
        ...
        self.sm.add_widget(HelpScreen(name='help')) # <-- ADICIONADO
        return self.sm
